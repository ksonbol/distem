...
#          raise Lib::UninitializedResourceError, vnode.name if vnode.status == Resource::Status::INIT
      ...
      # Parse mapping file generated by Alevin
      # ==== Attributes
      # *+file+ Text file
      # ==== Returns
      # Hash with the mapping read
      #
      def parse_mapping(file)
        # {:pnode1 => [vnode1,vnode2], :pnode2 => [vnode1,vnode2]}
        mapping = {}
        File.readlines(file).each do |line|
          nodes = line.split(/[-,]/).map{ |e| e.strip}
          pnode = nodes.shift
          nodes.each{ |vnode| mapping[vnode.to_sym] = pnode}
        end
        mapping
      end
      ...
      def updateobj_pnode(pnode, hash)
        ...
        hash['cpu']['critical_cache_links'].each do |l|
          pnode.cpu.add_critical_cache_link(l)
        end
      end
